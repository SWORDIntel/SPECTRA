...
class TestAttachmentForwarderGrouping(unittest.TestCase):
    ...
    def test_compute_message_hash_consistency(self):
        # Test that the same message content yields the same hash
        # and different content yields different hashes.
        now = datetime.now(tz=timezone.utc)
        msg1_content_a = MockMessage(id=1, date=now, sender_id=1, filename="fileA.txt", text="Hello")
        msg1_content_b = MockMessage(id=1, date=now, sender_id=1, filename="fileA.txt", text="Hello")
        msg2_content_c = MockMessage(id=2, date=now, sender_id=1, filename="fileA.txt", text="World")
        msg3_content_d = MockMessage(id=3, date=now, sender_id=1, filename="fileB.txt", text="Hello")

        hash1a = self.forwarder._compute_message_hash(msg1_content_a)
        hash1b = self.forwarder._compute_message_hash(msg1_content_b)
        hash2c = self.forwarder._compute_message_hash(msg2_content_c)
        hash3d = self.forwarder._compute_message_hash(msg3_content_d)

        self.assertEqual(hash1a, hash1b)
        self.assertNotEqual(hash1a, hash2c)
        self.assertNotEqual(hash1a, hash3d)
        self.assertNotEqual(hash2c, hash3d)

        # Test with slightly different media attributes
        msg_media1 = MockMessage(id=10, date=now, sender_id=1, filename="media.jpg")
        # msg_media1.media.id is set to 100 by mock logic (id*10)
        # msg_media1.media.access_hash is set to 1000 by mock logic (id*100)
        # msg_media1.file.id is set to 200 (id*20)
        # msg_media1.file.size is set based on filename length

        msg_media2 = MockMessage(id=11, date=now, sender_id=1, filename="media.jpg") # Different message ID
        # msg_media2.media.id = 110
        # msg_media2.media.access_hash = 1100
        # msg_media2.file.id = 220

        # Create a message that should be identical to msg_media1 for hashing purposes initially
        msg_media_identical_to_1 = MockMessage(id=10, date=now, sender_id=1, filename="media.jpg")
        # media.id=100, media.access_hash=1000, file.id=200, file.size matches

        # Now, create msg_media3 by taking the identical message and changing only media.access_hash
        msg_media3 = MockMessage(id=10, date=now, sender_id=1, filename="media.jpg")
        # All attributes are same as msg_media1 initially due to same id=10
        msg_media3.media.access_hash = 1001 # Explicitly change only media.access_hash

        hash_m1 = self.forwarder._compute_message_hash(msg_media1)
        hash_m_identical_to_1 = self.forwarder._compute_message_hash(msg_media_identical_to_1)
        hash_m2 = self.forwarder._compute_message_hash(msg_media2)
        hash_m3 = self.forwarder._compute_message_hash(msg_media3)

        self.assertNotEqual(hash_m1, hash_m2)
        self.assertNotEqual(hash_m1, hash_m3)


    def test_compute_group_hash_consistency(self):
        now = datetime.now(tz=timezone.utc)
        msg1 = MockMessage(id=1, date=now, sender_id=1, filename="file1.txt", text="A")
        msg2 = MockMessage(id=2, date=now, sender_id=1, filename="file2.txt", text="B")
        msg3 = MockMessage(id=3, date=now, sender_id=1, filename="file3.txt", text="C")

        group1 = [msg1, msg2]
        group1_ordered_differently = [msg2, msg1] # Order shouldn't matter for group hash
        group2 = [msg1, msg3]
        group3 = [msg1]

        hash_g1a = self.forwarder._compute_group_hash(group1)
        hash_g1b = self.forwarder._compute_group_hash(group1_ordered_differently)
        hash_g2 = self.forwarder._compute_group_hash(group2)
        hash_g3 = self.forwarder._compute_group_hash(group3)
        hash_g1_again = self.forwarder._compute_group_hash([
            MockMessage(id=1, date=now, sender_id=1, filename="file1.txt", text="A"),
            MockMessage(id=2, date=now, sender_id=1, filename="file2.txt", text="B")
        ])


        self.assertEqual(hash_g1a, hash_g1b, "Group hash should be consistent regardless of message order in input list.")
        self.assertEqual(hash_g1a, hash_g1_again, "Group hash should be consistent for identical groups.")
        self.assertNotEqual(hash_g1a, hash_g2, "Different groups should have different hashes.")
        self.assertNotEqual(hash_g1a, hash_g3, "Different groups (size) should have different hashes.")
        self.assertNotEqual(hash_g2, hash_g3, "Different groups should have different hashes.")

        # Test empty group
        self.assertEqual(self.forwarder._compute_group_hash([]), "", "Empty group should produce empty or defined hash.")
