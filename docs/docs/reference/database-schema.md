...
sidebar_position: 1
description: Complete database schema documentation for SPECTRA
...
Stores information about forwarded messages for deduplication purposes.

| Column          | Type    | Constraints | Description                                      |
|-----------------|---------|-------------|--------------------------------------------------|
| `id`            | INTEGER | PRIMARY KEY AUTOINCREMENT | Unique identifier for the forwarded message record. |
| `message_hash`  | TEXT    | NOT NULL    | Hash of the message content and media attributes. |
| `origin_id`     | TEXT    |             | Source channel/chat ID or username.              |
| `destination_id`| TEXT    |             | Destination channel/chat ID or username.         |
| `forwarded_at`  | TEXT    |             | ISO 8601 timestamp of when the message was forwarded. |
| `operation_type`| TEXT   |             | Type of operation (e.g., 'forward', 'shunt').    |

**Indexes:**
- `idx_forwarded_messages_hash` ON `forwarded_messages(message_hash)`

---

For more information on using the database, see the [Database Integration Guide](./database-integration.md).
